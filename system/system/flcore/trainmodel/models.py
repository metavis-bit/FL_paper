...
class VGG11(nn.Module):
    ...
    def forward(self, x, pt=None, is_observe=False, forward_flag=False):
        out = self.features(x)
        ...
        if pt is not None:
            # print(pt.shape)
            # print(out.shape)
            X_ = torch.cat([out, torch.flatten(pt, 1)], dim=1)
            avg_pool = nn.AvgPool1d(kernel_size=1+1, stride=1+1)
            X_ = avg_pool(X_.unsqueeze(1)).squeeze(1)

            out = torch.reshape(X_, out.shape)
