    ...
    def step(self, beta=0):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                #     continue
                d_p = p.grad.data
                if count < total_layer * decision_ratio:
                    count += 1
    ...
    def step(self, server_cs, client_cs):
        for group in self.param_groups:
            for p, sc, cc in zip(group['params'], server_cs, client_cs):
                p.data = p.data - group['lr'] * (p.grad.data + (sc - cc))
                # p.data.add_(other=(p.grad.data + sc - cc), alpha=-group['lr'])

class S_SCAFFOLDOptimizer(Optimizer):
    def __init__(self, params, lr=0):
        defaults = dict(lr=lr)
        super(S_SCAFFOLDOptimizer, self).__init__(params, defaults)

    def step(self, server_cs, client_cs, decision_l):
        for group in self.param_groups:
            count = 0
            for p, sc, cc in zip(group['params'], server_cs, client_cs):
                # # VGG_Cifar100
                # if count < total_layer - 2 * decision_l or count % 2 != 0:
                #     p.data.add_(other=(p.grad), alpha=-group['lr'])
                #     count += 1
                # else:
                #     p.data.add_(other=(p.grad + sc - cc), alpha=-group['lr'])
                #     count += 1

                # for general situation
                if count < total_layer - decision_l:
                    p.data = p.data - group['lr'] * (p.grad.data)
                    # p.data.add_(other=(p.grad), alpha=-group['lr'])
                    count += 1
                else:
    ...
    def step(self, global_params, device):
        for group in self.param_groups:
            for p, g in zip(group['params'], global_params):
                g = g.to(device)
                d_p = p.grad.data + group['mu'] * (p.data - g.data) ** 2
